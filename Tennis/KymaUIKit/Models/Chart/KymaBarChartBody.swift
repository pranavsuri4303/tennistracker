...
struct KymaBarChartBody: View {
    var type:KymaChartType
    var labels:[String]
    @ObservedObject var datasets:KymaBarDatasets
                ...
                VStack(spacing:0) {
                    ForEach(datasets.steps, id: \.self) { step in
                        HStack {
                            ...
                            .frame(width:30)
                            KymaDivider(orientation:.Horizontal, color:Color.clear)
                        }
...
//                                KymaChartLineShape(data:dataset.data, cellWidth: getCellWidth(width: geometryReader.size.width), cellHeight: getCellHeight(height: geometryReader.size.height), dataStep: datasets.dataStep, firstStep: datasets.steps.last!, tension: dataset.tension, filled: true)
//                                    .fill(dataset.fillColor.opacity(0.24))
//                            }
//
//                            if dataset.borderColor != Color.clear {
//                                KymaChartLineShape(data:dataset.data, cellWidth: getCellWidth(width: geometryReader.size.width), cellHeight: getCellHeight(height: geometryReader.size.height), dataStep: datasets.dataStep, firstStep: datasets.steps.last!, tension: dataset.tension)
