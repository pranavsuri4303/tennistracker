...
struct KymaDoughnutChartBody: View {
    var datasets:KymaDoughnutDatasets
    var background:Color = Color.white
    var emptyColor:Color = Color(KymaColor: .Gray11)
            ...
            ForEach(0..<datasets.datasets.count) { j in
                ZStack {
                    ...
                    ForEach(0..<datasets.datasets[j].data.count) { i in
                        KymaChartPieItemShape(startAngle: .init(degrees: datasets.startDegreeses[j][i]), endAngle: .init(degrees: datasets.endDegreeses[j][i]))
